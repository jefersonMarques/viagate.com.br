...
func (application *Application) sitemap(response http.ResponseWriter, request *http.Request) {
	...
	urls := []sitemapURL{
		{Location: baseURL + "/", Change: "weekly", Priority: "1.0"},
		{Location: baseURL + "/solucoes", Change: "monthly", Priority: "0.9"},
		{Location: baseURL + "/ferramentas", Change: "monthly", Priority: "0.9"},
		{Location: baseURL + "/analises", Change: "monthly", Priority: "0.9"},
		{Location: baseURL + "/sobre", Change: "monthly", Priority: "0.6"},
		{Location: baseURL + "/blog", Change: "weekly", Priority: "0.8"},
		{Location: baseURL + "/contato", Change: "monthly", Priority: "0.7"},
		{Location: baseURL + "/termos-uso-politica-privacidade", Change: "yearly", Priority: "0.2"},
		{Location: baseURL + "/integracoes/api", Change: "monthly", Priority: "0.8"},
		{Location: baseURL + "/white-label", Change: "monthly", Priority: "0.7"},
	}
	for _, solution := range content.Solutions() {
		if solution.Slug != "api" && solution.Slug != "white-label" {
			urls = append(urls, sitemapURL{Location: baseURL + "/solucoes/" + solution.Slug, Change: "monthly", Priority: "0.8"})
		}
	}
	for _, analysis := range content.Analyses() {
		urls = append(urls, sitemapURL{Location: baseURL + "/analises/" + analysis.Slug, Change: "monthly", Priority: "0.75"})
	}
